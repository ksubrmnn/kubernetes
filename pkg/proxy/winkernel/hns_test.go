...
func testCreateEndpointRemote(t *testing.T, hns HostNetworkService) {
	Network, err := createTestNetwork()
	if err != nil {
		t.Error(err)
	}
	...
	if endpoint.providerAddress != epPaAddress {
		t.Errorf("%v does not match %v", endpoint.providerAddress, epPaAddress)
	}
